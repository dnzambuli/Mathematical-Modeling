---
title: "ABM of epidemic spread: the effect of vacationers on a network"
...
**ATTRIBUTION: These codes are borrowed from Tauseef Ibn Mamun & Shane T. Mueller for there research work on Modeling disease spread with while accounting for the effect of vacationers on a network**
...
> **left Network**
>
> -   travelers who move in and out of the network
...
travel_ban_lift <- function( enable_outside_travel_restriction_day,second_wave_day,
    ...
    for(i in 1:length(sneezers))
    {
     ...
     if(plotNetwork)
     {
       ...
       if(need_to_update_graph_cord){
        ...
        daily_coords[[day]] <- list(cc, socialnetwork, states)
 ...
 # print(traveler_percentage_mean)
  ...
  return(list(disthistory = disthistory, coordhistory = daily_coords))
...
```{r,fig.width=10,fig.height=10,  fig.show='animate'}
 ...
 numDays = 100
  ...
  disthistory <- results$disthistory
  coordhistory <- results$coordhistory

  disthist.df <-as.data.frame(disthistory)
  #######################################################
  #
  ## create a coordinate history dataframe
  #
  #######################################################
  coordhist.df <- data.frame(day = integer(), x = numeric(), y = numeric(), state = numeric())
  # loop through each day
  for(day in seq_along(coordhistory)){
    coords <- coordhistory[[day]][[1]] # get the coordinates for each day
    states <- coordhistory[[day]][[3]]
    
    # Ensure there's a state for each coordinate
    if (length(states) != nrow(coords)) {
        states <- rep(NA, nrow(coords))  # Or handle this situation appropriately
    }
    
    temp_df <- data.frame(day = rep(day, nrow(coords)),
                          x = coords[, 1],
                          y = coords[, 2],
                          state = statesk)
    coordhist.df = rbind(coordhist.df, temp_df) # merge the dataframes
    
  }
  #######################################################
  #
  ## end of coordinate dataframe
  #
  #######################################################
